...
def main():
    ap = argparse.ArgumentParser(
        prog="incopilot",
        description="AI Incident Copilot CLI — collect logs, detect patterns, suggest safe next steps."
    )
    ...
    j.add_argument("--since", default="30 min ago", help="e.g. '30 min ago' or '2026-03-01 10:00:00'")

    d = sub.add_parser("docker", help="Collect from docker logs")
    d.add_argument("--container", required=True, help="container name or ID")
    d.add_argument("--since",     default="1h",  help="e.g. 1h, 30m, 2026-03-01T10:00:00")
    ...
    if args.source == "journal":
        text, target, since = collectors.from_journal(args.unit, args.since), args.unit, args.since
    elif args.source == "docker":
        text, target, since = collectors.from_docker(args.container, args.since), args.container, args.since
    elif args.source == "bundle":
        text   = collectors.bundle(args.unit, args.since_journal, args.container, args.since_docker)
        target = f"{args.unit} + {args.container}"
        since  = f"journal:{args.since_journal}  docker:{args.since_docker}"
    else:
        text, target, since = collectors.from_file(args.path), args.path, "n/a"
    ...
    paths               = write_outputs(args.out_dir, meta, findings, signal_hits, top_lines, suggestions)
